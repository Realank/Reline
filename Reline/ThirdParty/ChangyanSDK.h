...
//                                                    使用帮助
// 1. 名词解释:
// appID  : 畅言分配给第三方应用的app id，用来鉴别身份。登录changyan.sohu.com后，点击设置->高级。
// appKey : 畅言分配给第三方应用的app key，用来进行数据加密和签名校验。登录changyan.sohu.com后，点击设置->高级。
...
// redirectUrl : 使用oauth方式登录畅言的回调页面地址，登录changyan.sohu.com后，点击设置->高级，进行修改。
...
// 3. 快速接入事项
// 1) 导入依赖文件
//    将ChangYanSDK中libChangYanSDK.a、ChangYanSDK.h和changyan.bundle文件导入工程，并添加Security.framework。
// 2) 替换图片
